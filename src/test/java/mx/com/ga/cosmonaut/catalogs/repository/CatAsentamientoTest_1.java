...
@MicronautTest
public class CatAsentamientoTest_1 {

	private static final Logger LOG = LoggerFactory.getLogger(CatAsentamientoTest_1.class);
        
    @Inject
    @Client("/catalogo/catAsentamiento")
  ...
  /*  @Test
    ...
    public void findEsActivoTest() {
        final RespuestaGenerica respuesta =
                cliente.toBlocking().retrieve(HttpRequest.GET("/listar/todosActivo/" + Boolean.FALSE),
                        RespuestaGenerica.class);
        assertTrue(respuesta.isResultado());
        assertTrue(Constantes.EXITO.equals(respuesta.getMensaje()));
        LOG.info("Respuesta {}", respuesta.getDatos());
